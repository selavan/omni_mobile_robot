#'''
#adjusting the number of waypoint in trajectory by epsilon
#A higher epsilon value will result in fewer waypoints
import math
import rclpy
...
def rdp(points, epsilon):
    ...
    def rdp_rec(start_index, end_index):
        ...
        for i in range(start_index + 1, end_index):
            distance = perpendicular_distance(points[i], points[start_index], points[end_index])
            if distance > max_distance:
                index = i
                max_distance = distance

        if max_distance > epsilon:
            results1 = rdp_rec(start_index, index)
            results2 = rdp_rec(index, end_index)
            return results1 + [points[index]] + results2
        else:
            return []

    return [points[0]] + rdp_rec(0, len(points) - 1) + [points[-1]]
...
def main(args=None):
    ...
    global initial_pose
    global goal
    global new_goal_received
    global path_msg  # Store the path message globally

    initial_pose = None
    goal = None
    new_goal_received = False
    path_msg = None  # Initialize the path message
    ...
    while rclpy.ok():
        ...
        if planner.obstacle_map is not None and initial_pose is not None and goal is not None and new_goal_received:
            rx, ry = planner.planning(initial_pose, goal)

            # Simplify the path using the RDP algorithm
            path_points = list(zip(rx, ry))
            simplified_path = rdp(path_points, epsilon=0.05)  # Adjust epsilon as needed
            #A higher epsilon value will result in fewer waypoints
            ...
            new_goal_received = False  # Reset the flag

        if path_msg is not None:
            path_pub.publish(path_msg)  # Publish the stored path message
