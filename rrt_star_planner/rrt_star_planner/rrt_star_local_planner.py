...
class RRTStarNode(Node):
    ...
    def local_map_callback(self, msg):
        ...
        self.get_logger().info('Local map with dynamic obstacles received.')
        self.path_found = False  # Trigger replanning
    ...
    def is_near_obstacle(self, point, map_data):
        if map_data is None:
            return False
        ...
        for dx in range(-search_radius, search_radius + 1):
            for dy in range(-search_radius, search_radius + 1):
                nx = pixel_x + dx
                ny = pixel_y + dy
                if nx >= 0 and nx < map_data.shape[1] and ny >= 0 and ny < map_data.shape[0]:
                    if map_data[ny, nx] == 100:
                        return True
        return False
    ...
    def run_rrt_star(self):
        if self.start is None or self.goal is None:
            self.get_logger().info('Waiting for start and goal positions...')
            return
        if self.global_map_data is None and self.local_map_data is None:
            self.get_logger().info('Waiting for map data...')
            return

        map_data = self.local_map_data if self.local_map_data is not None else self.global_map_data
        ...
        for j in range(self.max_iter):
            ...
            if self.is_in_obstacle(x_rand, map_data) or self.is_near_obstacle(x_rand, map_data):
                continue
            ...
            if self.is_in_obstacle(x_new, map_data) or self.is_near_obstacle(x_new, map_data):
                continue
